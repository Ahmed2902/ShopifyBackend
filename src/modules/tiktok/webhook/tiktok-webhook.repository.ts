...
export class TikTokWebhookRepository {
  findConnectionByAdvertiserId(advertiserId: string) {
    return prisma.tikTokAdvertiser.findFirst({
      where: { advertiserId },
    ...
    try {
      const delivery = await prisma.webhookDelivery.create({
        data: {
          ...
          lastError: input.connectionId ? null : 'No configured TikTok advertiser matched this delivery',
  ...
  markProcessed(id: string) {
    return prisma.webhookDelivery.update({
      where: { id },
      data: { status: 'PROCESSED', processedAt: new Date(), processingStartedAt: null, nextAttemptAt: null, lastError: null },
    });
  }
  ...
  markIgnored(id: string, reason: string) {
    return prisma.webhookDelivery.update({
      where: { id },
      data: { status: 'IGNORED', processedAt: new Date(), processingStartedAt: null, nextAttemptAt: null, lastError: reason.slice(0, 4000) },
    });
  }
