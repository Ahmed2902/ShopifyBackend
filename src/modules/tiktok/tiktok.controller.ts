...
import { buildTikTokSuccessRedirect } from './tiktok.utils.js';
...
export class TikTokController {
  ...
  completeInstall = async (req: Request, res: Response) => {
    if (typeof req.query.error === 'string') {
      throw new AppError(
        typeof req.query.error_description === 'string' ? req.query.error_description : 'TikTok authorization was not completed',
        400,
        'TIKTOK_OAUTH_DENIED',
      );
    }
    const query = tiktokCallbackSchema.parse({ auth_code: req.query.auth_code, code: req.query.code, state: req.query.state });
  ...
  status = async (req: Request, res: Response) => res.status(200).json(await this.service.getStatus(req.context.storeId!));
  assets = async (req: Request, res: Response) => res.status(200).json(await this.service.discoverAssets(req.context.storeId!));

  configure = async (req: Request, res: Response) => {
    res.status(200).json(await this.service.configureAssets(req.context.storeId!, tiktokConfigureAssetsSchema.parse(req.body)));
  };
  ...
  sync = async (req: Request, res: Response) => res.status(200).json(await this.service.syncAdsHierarchy(req.context.storeId!));
  syncCatalogs = async (req: Request, res: Response) => res.status(200).json(await this.service.syncCatalogs(req.context.storeId!));
  ...
  campaigns = async (req: Request, res: Response) => {
    res.status(200).json(await this.service.listCampaigns(req.context.storeId!, tiktokCampaignListQuerySchema.parse(req.query)));
  };

  adGroups = async (req: Request, res: Response) => {
    res.status(200).json(await this.service.listAdGroups(req.context.storeId!, tiktokAdGroupListQuerySchema.parse(req.query)));
  };

  ads = async (req: Request, res: Response) => {
    res.status(200).json(await this.service.listAds(req.context.storeId!, tiktokAdListQuerySchema.parse(req.query)));
  };
  ...
  catalogs = async (req: Request, res: Response) => res.status(200).json(await this.service.listCatalogs(req.context.storeId!));
  ...
  insights = async (req: Request, res: Response) => {
    res.status(200).json(await this.service.listInsights(req.context.storeId!, tiktokInsightsListQuerySchema.parse(req.query)));
  };
  ...
  replaceAdMapping = async (req: Request, res: Response) => {
    ...
    res.status(200).json(await this.service.setManualAdMapping(req.context.storeId!, adId, input.productId, input.variantId ?? null));
  };

  replaceCatalogItemMapping = async (req: Request, res: Response) => {
    const catalogItemId = req.params.catalogItemId;
    if (!catalogItemId) throw new AppError('Catalog item ID is required', 400, 'INVALID_CATALOG_ITEM_ID');
    const { variantId } = tiktokManualCatalogMappingSchema.parse(req.body);
    res.status(200).json(await this.service.setManualCatalogMapping(req.context.storeId!, catalogItemId, variantId));
  };
}
