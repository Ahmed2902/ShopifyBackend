...
import {
  ...
  recordOperationPrismaQuery,
} from '../../src/observability/operation-performance.js';
...
describe('background operation performance', () => {
  it('records Prisma query count and union wall time for a provider sync', async () => {
    ...
    await profileBackgroundOperation('provider_sync', { provider: 'SHOPIFY' }, async () => {
      recordOperationPrismaQuery({
        model: 'Order',
        operation: 'findMany',
        durationMs: 25,
  ...
  it('logs failed operations before rethrowing them', async () => {
    ...
    await expect(
      profileBackgroundOperation('provider_sync', { provider: 'SHOPIFY', mode: 'MANUAL' }, async () => {
