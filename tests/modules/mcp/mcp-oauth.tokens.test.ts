...
function repository() {
  ...
  return {
    ...
    claimAuthorizationRequestAndCreateCode: vi.fn(),
    findAuthorizationCode: vi.fn().mockResolvedValue(code),
    consumeAuthorizationCode: vi.fn().mockResolvedValue(code),
...
describe('McpOAuthService tokens', () => {
  it('validates PKCE and bindings before consuming a one-time authorization code', async () => {
  ...
  it('rejects a code when the merchant no longer belongs to the granted store', async () => {
    const repo = repository();
  ...
  it('rejects refresh when current store membership has been revoked before rotating', async () => {
    const repo = repository();
